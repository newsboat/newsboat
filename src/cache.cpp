...
static int lastmodified_callback(void * handler, int argc, char ** argv, char ** /* azColName */) {
	...
	LOG(level::INFO, "lastmodified_callback: lastmodified = %d etag = %s", result->lastmodified, result->etag.c_str());
	return 0;
}
...
cache::cache(const std::string& cachefile, configcontainer * c) : db(0),cfg(c) {
	...
	if (error != SQLITE_OK) {
		LOG(level::ERROR,"couldn't sqlite3_open(%s): error = %d", cachefile.c_str(), error);
...
void cache::set_pragmas() {
	...
	if (rc != SQLITE_OK) {
		LOG(level::CRITICAL,"setting PRAGMA synchronous = OFF failed");
		throw dbexception(db);
	}
	...
	if (rc != SQLITE_OK) {
		LOG(level::CRITICAL,"setting PRAGMA case_sensitive_like = OFF failed");
		throw dbexception(db);
	}
...
void cache::populate_tables() {
	...
	LOG(level::DEBUG, "cache::populate_tables: CREATE TABLE rss_feed rc = %d", rc);
	...
	LOG(level::DEBUG, "cache::populate_tables: CREATE TABLE rss_item rc = %d", rc);
	...
	LOG(level::DEBUG, "cache::populate_tables: CREATE TABLE google_replay rc = %d", rc);
...
void cache::fetch_lastmodified(const std::string& feedurl, time_t& t, std::string& etag) {
	...
	if (rc != SQLITE_OK) {
		LOG(level::CRITICAL, "query \"%s\" failed: error = %d", query.c_str(), rc);
		throw dbexception(db);
	}
...
void cache::update_lastmodified(const std::string& feedurl, time_t t, const std::string& etag) {
	if (t == 0 && etag.length() == 0) {
		LOG(level::INFO, "cache::update_lastmodified: both time and etag are empty, not updating anything");
		return;
	}
	std::lock_guard<std::mutex> lock(mtx);
	...
	LOG(level::DEBUG, "ran SQL statement: %s result = %d", query.c_str(), rc);
}
...
void cache::mark_item_deleted(const std::string& guid, bool b) {
	...
	int rc = sqlite3_exec(db, query.c_str(), nullptr, nullptr, nullptr);
	LOG(level::DEBUG, "cache::mark_item_deleted ran SQL statement: %s result = %d", query.c_str(), rc);
}
...
void cache::externalize_rssfeed(std::shared_ptr<rss_feed> feed, bool reset_unread) {
	...
	if (rc != SQLITE_OK) {
		LOG(level::CRITICAL,"query failed: error = %d", rc);
		throw dbexception(db);
	}

	int count = count_cbh.count();
	LOG(level::DEBUG, "cache::externalize_rss_feed: rss_feeds with rssurl = '%s': found %d",feed->rssurl().c_str(), count);
	if (count > 0) {
		...
		if (rc != SQLITE_OK) {
			LOG(level::CRITICAL,"query \"%s\" failed: error = %d", updatequery.c_str(), rc);
			throw dbexception(db);
		}
		LOG(level::DEBUG,"ran SQL statement: %s", updatequery.c_str());
	} else {
		...
		if (rc != SQLITE_OK) {
			LOG(level::CRITICAL,"query \"%s\" failed: error = %d", insertquery.c_str(), rc);
			throw dbexception(db);
		}
		LOG(level::DEBUG,"ran SQL statement: %s", insertquery.c_str());
	}

	unsigned int max_items = cfg->get_configvalue_as_int("max-items");

	LOG(level::INFO, "cache::externalize_feed: max_items = %u feed.total_item_count() = %u", max_items, feed->total_item_count());
...
void cache::do_vacuum() {
	...
	if (rc != SQLITE_OK) {
		LOG(level::CRITICAL,"query \"%s\" failed: error = %d", vacuum_query, rc);
	}
}
...
void cache::cleanup_cache(std::vector<std::shared_ptr<rss_feed>>& feeds) {
	...
	if (cfg->get_configvalue_as_bool("cleanup-on-quit")) {
		...
		LOG(level::DEBUG,"running query: %s", cleanup_rss_feeds_statement.c_str());
		rc = sqlite3_exec(db,cleanup_rss_feeds_statement.c_str(),nullptr,nullptr,nullptr);
		if (rc != SQLITE_OK) {
			LOG(level::CRITICAL,"query \"%s\" failed: error = %d", cleanup_rss_feeds_statement.c_str(), rc);
			throw dbexception(db);
		}

		LOG(level::DEBUG,"running query: %s", cleanup_rss_items_statement.c_str());
		rc = sqlite3_exec(db,cleanup_rss_items_statement.c_str(),nullptr,nullptr,nullptr);
		if (rc != SQLITE_OK) {
			LOG(level::CRITICAL,"query \"%s\" failed: error = %d", cleanup_rss_items_statement.c_str(), rc);
			throw dbexception(db);
		}

		if (cfg->get_configvalue_as_bool("delete-read-articles-on-quit")) {
			LOG(level::DEBUG,"running query: %s", cleanup_read_items_statement.c_str());
			rc = sqlite3_exec(db,cleanup_read_items_statement.c_str(),nullptr,nullptr,nullptr);
			if (rc != SQLITE_OK) {
				LOG(level::CRITICAL,"query \"%s\" failed: error = %d", cleanup_read_items_statement.c_str(), rc);
				throw dbexception(db);
			}
		}
	...
	if (count_cbh.count() > 0) {
		if (reset_unread) {
			...
			if (content != item->description_raw()) {
				LOG(level::DEBUG, "cache::update_rssitem_unlocked: '%s' is different from '%s'", content.c_str(), item->description_raw().c_str());
		...
		LOG(level::DEBUG,"running query: %s", update.c_str());
		rc = sqlite3_exec(db,update.c_str(),nullptr,nullptr,nullptr);
		if (rc != SQLITE_OK) {
			LOG(level::CRITICAL,"query \"%s\" failed: error = %d", update.c_str(), rc);
			throw dbexception(db);
		}
	} else {
		...
		LOG(level::DEBUG,"running query: %s", insert.c_str());
		rc = sqlite3_exec(db,insert.c_str(),nullptr,nullptr,nullptr);
		if (rc != SQLITE_OK) {
			LOG(level::CRITICAL,"query \"%s\" failed: error = %d", insert.c_str(), rc);
			throw dbexception(db);
		}
	}
}
...
void cache::remove_old_deleted_items(const std::string& rssurl, const std::vector<std::string>& guids) {
	...
	if (guids.size() == 0) {
		LOG(level::DEBUG, "cache::remove_old_deleted_items: not cleaning up anything because last reload brought no new items (detected no changes)");
		return;
	}
	std::string guidset = "(";
	for (auto guid : guids) {
	...
	if (rc != SQLITE_OK) {
		...
		LOG(level::DEBUG, "cache::remove_old_deleted_items: executed query successfully: %s", query.c_str());
	}
}
...
unsigned int cache::get_unread_count() {
	...
	LOG(level::DEBUG, "cache::get_unread_count: rc = %d count = %u", rc, count);
	return count;
}
...
void cache::mark_items_read_by_guid(const std::vector<std::string>& guids) {
	...
	if (rc != SQLITE_OK) {
		...
		LOG(level::DEBUG, "cache::mark_items_read_by_guid: executed query successfully: %s", updatequery.c_str());
	}
}
...
std::vector<std::string> cache::get_read_item_guids() {
	...
	if (rc != SQLITE_OK) {
		...
		LOG(level::DEBUG, "cache::get_read_item_guids: executed query successfully: %s", query.c_str());
	}
	return guids;
}
...
void cache::clean_old_articles() {
	...
	if (days > 0) {
		...
		LOG(level::DEBUG, "cache::clean_old_articles: about to delete articles with a pubDate older than %d", old_date);
		int rc = sqlite3_exec(db, query.c_str(), nullptr, nullptr, nullptr);
		LOG(level::DEBUG, "cache::clean_old_artgicles: old article delete result: rc = %d", rc);
	} else {
