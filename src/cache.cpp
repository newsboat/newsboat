...
static int single_string_callback(
	void* handler,
	int argc,
	char** argv,
	char** /* azColName */)
...
static int lastmodified_callback(
	void* handler,
	int argc,
	char** argv,
	char** /* azColName */)
...
static int fill_content_callback(
	void* myfeed,
	int argc,
	char** argv,
	char** /* azColName */)
...
static int search_item_callback(
	void* myfeed,
	int argc,
	char** argv,
	char** /* azColName */)
	...
	{{2, 10},
	 {
		 ...
		 /* we need to do these ALTER TABLE statements because we need
		  * to store additional data for the podcast support */
		 ...
		 "ALTER TABLE rss_feed ADD lastmodified INTEGER(11) NOT NULL "
		 "DEFAULT 0;",

		 "CREATE INDEX IF NOT EXISTS idx_lastmodified ON "
		 "rss_feed(lastmodified);",

		 "ALTER TABLE rss_item ADD deleted INTEGER(1) NOT NULL DEFAULT "
		 "0;",

		 "CREATE INDEX IF NOT EXISTS idx_deleted ON rss_item(deleted);",

		 "ALTER TABLE rss_feed ADD is_rtl INTEGER(1) NOT NULL DEFAULT "
		 "0;",
...
void cache::update_lastmodified(
	const std::string& feedurl,
	time_t t,
...
void cache::externalize_rssfeed(
	std::shared_ptr<rss_feed> feed,
	bool reset_unread)
{
	...
	LOG(level::DEBUG,
	    "cache::externalize_rss_feed: rss_feeds with rssurl = '%s': found "
	    "%d",
	    feed->rssurl(),
	    count);
	if (count > 0) {
	...
	LOG(level::INFO,
	    "cache::externalize_feed: max_items = %u feed.total_item_count() = "
	    "%u",
	    max_items,
...
cache::internalize_rssfeed(std::string rssurl, rss_ignores* ign)
{
	...
	if (count_cbh.count() > 0) {
		if (reset_unread) {
			...
			if (content != item->description_raw()) {
				LOG(level::DEBUG,
				    "cache::update_rssitem_unlocked: '%s' is "
				    "different from '%s'",
				    content,
