...
time_t rss_parser::parse_date(const std::string& datestr)
{
	...
	if (t == -1) {
		LOG(level::INFO,
		    "rss_parser::parse_date: encountered t == -1, trying out "
		    "W3CDTF parser...");
		t = curl_getdate(
	...
	if (t == -1) {
		LOG(level::INFO,
		    "rss_parser::parse_date: still t == -1, setting to current "
		    "time");
		t = ::time(nullptr);
	}
	return t;
...
std::string rss_parser::render_xhtml_title(
	...
	const std::string& lang)
{
	...
	if (it != rtl_langprefix.end()) {
		LOG(level::DEBUG,
		    "rss_parser::parse: detected right-to-left order, language "
		    "code = %s",
		    *it);
		feed->set_rtl(true);
	}
...
void rss_parser::retrieve_uri(const std::string& uri)
{
	...
	if (is_ttrss) {
		...
		uri.substr(0, 5) == "http:" || uri.substr(0, 6) == "https:") {
		download_http(uri);
	...
	for (unsigned int i = 0; i < retrycount && !is_valid; i++) {
		try {
			...
			if (p.get_last_modified() != 0
			    || p.get_etag().length() > 0) {
				LOG(level::DEBUG,
				    "rss_parser::download_http: lastmodified "
				    "old: %d new: %d",
				    lm,
				    p.get_last_modified());
				LOG(level::DEBUG,
				    "rss_parser::download_http: etag old: %s "
				    "new %s",
				    etag,
				    p.get_etag());
				ch->update_lastmodified(
					uri,
...
void rss_parser::download_filterplugin(
...
void rss_parser::fill_feed_items(std::shared_ptr<rss_feed> feed)
{
	...
	for (auto item : f.items) {
		...
		LOG(level::DEBUG,
		    "rss_parser::parse: item title = `%s' link = `%s' pubDate "
		    "= `%s' (%d) description = `%s'",
		    x->title(),
		    x->link(),
		    x->pubDate(),
...
void rss_parser::set_item_title(
	std::shared_ptr<rss_feed> feed,
...
void rss_parser::set_item_enclosure(
	std::shared_ptr<rss_item> x,
	rsspp::item& item)
{
...
void rss_parser::add_item_to_feed(
	std::shared_ptr<rss_feed> feed,
	std::shared_ptr<rss_item> item)
{
	...
	if (!ign || !ign->matches(item.get())) {
		feed->add_item(item);
		LOG(level::INFO,
		    "rss_parser::parse: added article title = `%s' link = `%s' "
		    "ign = %p",
		    item->title(),
		    item->link(),
		    ign);
...
void rss_parser::handle_itunes_summary(
	std::shared_ptr<rss_item> x,
	rsspp::item& item)
{
	if (x->description() != "")
