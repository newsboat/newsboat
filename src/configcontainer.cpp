...
configcontainer::configcontainer()
	...
	: config_data{
		  ...
		  {"articlelist-title-format",
		   configdata(
			   _("%N %V - Articles in feed '%T' (%u unread, %t "
			     "total) - %U"),
			   configdata_t::STR)},
		  ...
		  {"feedlist-title-format",
		   configdata(
			   _("%N %V - Your feeds (%u unread, %t total)%?T? - "
			     "tag `%T'&?"),
			   configdata_t::STR)},
...
void configcontainer::handle_action(
...
void configcontainer::set_configvalue(
...
void configcontainer::toggle(const std::string& key)
{
	if (config_data[key].type == configdata_t::BOOL) {
		set_configvalue(
			key,
			std::string(
				get_configvalue_as_bool(key) ? "false"
							     : "true"));
	}
}
...
configcontainer::get_suggestions(const std::string& fragment)
{
