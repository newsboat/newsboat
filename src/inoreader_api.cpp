...
inoreader_api::inoreader_api(configcontainer * c) : remote_api(c) {
	// TODO
}
...
std::string inoreader_api::retrieve_auth() {
	...
	std::string postcontent = strprintf::fmt("Email=%s&Passwd=%s", username, password);
...
std::vector<tagged_feedurl> inoreader_api::get_subscribed_urls() {
	...
	LOG(level::DEBUG, "inoreader_api::get_subscribed_urls: document = %s", result);

	json_object* reply = json_tokener_parse(result.c_str());
	if (reply == nullptr) {
		LOG(level::ERROR, "inoreader_api::get_subscribed_urls: failed to parse response as JSON.");
		return urls;
	}
...
void inoreader_api::add_custom_headers(curl_slist** custom_headers) {
	...
	LOG(level::DEBUG, "inoreader_api::add_custom_headers header = %s", auth_header);
...
bool inoreader_api::mark_all_read(const std::string& feedurl) {
	std::string real_feedurl = feedurl.substr(strlen(INOREADER_FEED_PREFIX));
	...
	std::string result = post_content(INOREADER_API_MARK_ALL_READ_URL, postcontent);
...
bool inoreader_api::mark_article_read_with_token(const std::string& guid, bool read, const std::string& token) {
	...
	std::string result = post_content(INOREADER_API_EDIT_TAG_URL, postcontent);

	LOG(level::DEBUG, "inoreader_api::mark_article_read_with_token: postcontent = %s result = %s", postcontent, result);
...
bool inoreader_api::update_article_flags(const std::string& inoflags, const std::string& newflags, const std::string& guid) {
	...
	if (star_flag.length() > 0) {
		if (strchr(inoflags.c_str(), star_flag[0])==nullptr && strchr(newflags.c_str(), star_flag[0])!=nullptr) {
			success = star_article(guid, true);
		} else if (strchr(inoflags.c_str(), star_flag[0])!=nullptr && strchr(newflags.c_str(), star_flag[0])==nullptr) {
			success = star_article(guid, false);
		}
	}

	if (share_flag.length() > 0) {
		if (strchr(inoflags.c_str(), share_flag[0])==nullptr && strchr(newflags.c_str(), share_flag[0])!=nullptr) {
			success = share_article(guid, true);
		} else if (strchr(inoflags.c_str(), share_flag[0])!=nullptr && strchr(newflags.c_str(), share_flag[0])==nullptr) {
...
std::string inoreader_api::post_content(const std::string& url, const std::string& postdata) {
	std::string result;
	curl_slist* custom_headers {};
	...
	LOG(level::DEBUG, "inoreader_api::post_content: url = %s postdata = %s result = %s", url, postdata, result);
