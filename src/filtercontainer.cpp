...
void filtercontainer::handle_action(const std::string& action, const std::vector<std::string>& params) {
	...
	if (action == "define-filter") {
		...
		matcher m;
		if (!m.parse(params[1]))
			throw confighandlerexception(utils::strprintf(_("couldn't parse filter expression `%s': %s"), params[1], m.get_parse_error()));
...
void filtercontainer::dump_config(std::vector<std::string>& config_output) {
	for (auto filter : filters) {
		config_output.push_back(utils::strprintf("define-filter %s %s", utils::quote(filter.first), utils::quote(filter.second)));
	}
}
