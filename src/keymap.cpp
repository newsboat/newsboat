...
static const std::map<Dialog, std::uint32_t> contexts = {
	...
	{Dialog::SearchResultList, KM_SEARCHRESULTSLIST},
};
...
void KeyMap::set_key(Operation op,
	...
	std::optional<Dialog> context)
{
	LOG(Level::DEBUG, "KeyMap::set_key(%d,%s) called", op, key.to_bindkey_string());
	if (context.has_value()) {
		apply_bindkey(context_keymaps[context.value()], key, op);
	} else {
		for (const auto& ctx : contexts) {
...
void KeyMap::unset_key(const KeyCombination& key, std::optional<Dialog> context)
{
	LOG(Level::DEBUG, "KeyMap::unset_key(%s) called", key.to_bindkey_string());
	if (context.has_value()) {
		context_keymaps[context.value()].continuations.erase(key);
	} else {
		for (const auto& ctx : contexts) {
...
void KeyMap::unset_all_keys(std::optional<Dialog> context)
{
	LOG(Level::DEBUG, "KeyMap::unset_all_keys(%s) called",
		context.has_value() ? dialog_name(context.value()) : "");
	auto internal_ops_only = get_internal_operations();
	if (context.has_value()) {
		context_keymaps[context.value()] = std::move(internal_ops_only);
	} else {
		for (const auto& ctx : contexts) {
...
void KeyMap::handle_action(const std::string& action, const std::string& params)
{
	...
	if (action == "bind-key") {
		...
		if (tokens.size() >= 3 && tokens[2] != "all") {
			context = dialog_from_name(tokens[2]);
		...
		if (tokens.size() >= 2 && tokens[1] != "all") {
			context = dialog_from_name(tokens[1]);
			if (!context.has_value()) {
				throw ConfigHandlerException(strprintf::fmt(
