	...
	const std::string& context)
{
	LOG(Level::DEBUG,
		...
		context);
	const auto& mappings = context_keymaps.at(context);
	if (mappings.is_leaf_node) {
		return OP_NIL;
	}
	if (mappings.continuations.count(key_combination) < 1) {
		return OP_NIL;
	}
	const auto& mapping = mappings.continuations.at(key_combination);
	if (!mapping.is_leaf_node) {
		return OP_NIL;
	}
	auto& cmds = mapping.action.cmds;
