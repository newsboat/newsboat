...
void keymap::get_keymap_descriptions(
		...
		for (unsigned int j = 0; opdescs[j].op != OP_NIL; ++j) {
			...
			if (!already_added) {
				if (opdescs[j].flags & flags) {
					LOG(level::DEBUG,
					    "keymap::get_keymap_descriptions: "
					    "found unbound function: %s ctx = "
					    "%s",
					    opdescs[j].opstr,
					    ctx);
					keymap_desc desc;
...
operation
keymap::get_operation(const std::string& keycode, const std::string& context)
{
	std::string key;
	LOG(level::DEBUG,
	...
	if (action == "bind-key") {
		...
		while (it != params.end()) {
			if (first && *it != ";") {
				...
				LOG(level::DEBUG,
				    "keymap::handle_action: new operation `%s' "
				    "(op = %u)",
				    it,
				    tmpcmd.op);
				if (tmpcmd.op == OP_NIL)
