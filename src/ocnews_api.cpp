...
ocnews_api::ocnews_api(configcontainer* c) : remote_api(c) {
	...
	if (cfg->get_configvalue("ocnews-login").empty() || cfg->get_configvalue("ocnews-password").empty())
		LOG(LOG_CRITICAL, "ocnews_api::ocnews_api: No user and/or password set");
	if (server.empty())
		LOG(LOG_CRITICAL, "ocnews_api::ocnews_api: No owncloud server set");
}
...
std::vector<tagged_feedurl> ocnews_api::get_subscribed_urls() {
	...
	for (int i=0; i<folders_length; i++) {
		json_object* folder = static_cast<json_object*>(folders_list->array[i]);
		json_object* node;
	...
	for (int i=0; i<feeds_length; i++) {
		json_object* feed = static_cast<json_object*>(feeds_list->array[i]);
		json_object* node;
...
bool ocnews_api::update_article_flags(const std::string& oldflags, const std::string& newflags, const std::string& guid) {
...
rsspp::feed ocnews_api::fetch_feed(const std::string& feed_id) {
	...
	if (json_object_get_type(items) != json_type_array) {
		LOG(LOG_ERROR, "ocnews_api::fetch_feed: items is not an array");
		return feed;
	}
...
bool ocnews_api::query(const std::string& query, json_object** result, const std::string& post) {
	...
	static auto write_fn = [] (void *buffer, size_t size, size_t nmemb, void *userp) {
	...
	if (response_code != 200) {
		if (response_code == 401)
			LOG(LOG_CRITICAL, "ocnews_api::query: authentication error");
		else {
