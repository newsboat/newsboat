...
class test_api : public remote_api {
	public:
		test_api(configcontainer *c) : remote_api(c) {}
		...
		bool authenticate() { throw 0; }
		std::vector<tagged_feedurl> get_subscribed_urls() { throw 0; }
		...
		bool update_article_flags(const std::string&, const std::string&, const std::string&) {
			throw 0;
		}
};

TEST_CASE("get_credentials() returns the users name and password", "[remote_api]") {
	configcontainer cfg;
...
TEST_CASE("read_password() returns the first line of the file", "[remote_api]") {
	...
	REQUIRE_NOTHROW(remote_api::read_password("a-passwordfile-that-is-guaranteed-to-not-exist.txt"));
	REQUIRE(remote_api::read_password("a-passwordfile-that-is-guaranteed-to-not-exist.txt") == "");
	REQUIRE(remote_api::read_password("data/single-line-string.txt") == "single line with spaces");
	REQUIRE(remote_api::read_password("data/multi-line-string.txt") == "string with spaces");
}

TEST_CASE("eval_password() returns the first line of command's output", "[remote_api]") {
	...
	REQUIRE(remote_api::eval_password("echo ' aaa \naaa'") == " aaa ");
	REQUIRE(remote_api::eval_password("echo '\naaa'") == "");
	REQUIRE_NOTHROW(remote_api::eval_password("a-program-that-is-guaranteed-to-not-exists"));
	REQUIRE(remote_api::eval_password("a-program-that-is-guaranteed-to-not-exists") == "");
	REQUIRE_NOTHROW(remote_api::eval_password("printf 'string with no newline'"));
	REQUIRE(remote_api::eval_password("printf 'string with no newline'") == "string with no newline");
	...
	// test cases that would require user input can not be covered at the moment
	// and would require a redesign of this method with a sole purpose to make it testable.
	// see:
	// https://github.com/akrennmair/newsbeuter/pull/503#issuecomment-282491118
	//
