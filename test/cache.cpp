...
TEST_CASE("Cleaning old articles works", "[Cache]")
{
	...
	const std::string uri = "file://data/rss.xml";
	FeedRetriever feed_retriever(*cfg, *rsscache);
	RssParser parser(uri, rsscache.get(), cfg.get(), nullptr);
	auto feed = parser.parse(feed_retriever.retrieve(uri));
	...
	"[Cache]")
{
	...
	FeedRetriever feed_retriever(cfg, *rsscache);
	std::unique_ptr<RssParser> parser(new RssParser(uri1, rsscache.get(), &cfg, nullptr));
	...
	parser.reset(new RssParser(uri2, rsscache.get(), &cfg, nullptr));
...
TEST_CASE("do_vacuum doesn't throw an exception", "[Cache]")
{
	...
	const std::string uri = "file://data/rss.xml";
	FeedRetriever feed_retriever(cfg, *rsscache);
	RssParser parser(uri, rsscache.get(), &cfg, nullptr);
	auto feed = parser.parse(feed_retriever.retrieve(uri));
	rsscache->externalize_rssfeed(feed, false);
