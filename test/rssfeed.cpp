...
TEST_CASE("RssFeed::sort() correctly sorts articles", "[rss]")
{
	...
	SECTION("title")
	{
		...
		f.sort(ss);
		articles = f.items();
		REQUIRE(articles[0]->title_raw() ==
			"Article 1: A boring article");
		REQUIRE(articles[1]->title_raw() ==
			"Article 2: Article you must read");
		REQUIRE(articles[2]->title_raw() ==
			"Article 10: Another great article");
		...
		REQUIRE(articles[2]->title_raw() ==
			"Article 10: Another great article");
		REQUIRE(articles[3]->title_raw() ==
			"Article 2: Article you must read");
		REQUIRE(articles[4]->title_raw() ==
			"Article 1: A boring article");
	}
...
TEST_CASE("If item's <title> is empty, try to deduce it from the URL", "[rss]")
{
	ConfigContainer cfg;
	Cache rsscache(":memory:", &cfg);
