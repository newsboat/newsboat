...
[[podlist-format-S]]<<podlist-format-S,+S+>>:Status of download, refer to the table <<available-podcast-states,+Available Podcast States+>> for all possible statuses
...
The following table lists all possible states and their meaning:

.Available Podcast States
[[available-podcast-states]]
[frame="all", grid="all", format="dsv", options="header", cols="30,70"]
|======================================================================
State:Meaning
...
[[dl-status-ready]]<<dl-status-ready,+ready+>>:The downloading process was successful and is ready to be played
[[dl-status-canceled]]<<dl-status-canceled,+canceled+>>:<<pb-cancel,+pb-cancel+>> has been called while downloading and will stop that process
[[dl-status-deleted]]<<dl-status-deleted,+deleted+>>:Will be removed from queue file upon exit Podboat or by calling <<pb-purge,+pb-purge+>>
[[dl-status-played]]<<dl-status-played,+played+>>:Set after a <<pb-play,+pb-play+>> call and the player is run. As of Newsboat version 2.44, the return code of the player is not checked.
[[dl-status-finished]]<<dl-status-finished,+finished+>>:Manually marked using <<pb-mark-as-finished,+pb-mark-as-finished+>>
[[dl-status-missing]]<<dl-status-missing,+missing+>>:The podcast was downloaded successfully but is no longer at the expected location
