...
Alternatively, key combinations using shift or control can be written like
...
the configued browser to `firefox` and then run the `open-in-browser` command
to open the feed/article in the configured browser.
