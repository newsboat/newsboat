...
Note that `:set` only changes configuration *variables* (e.g. `reload-time`,
`browser`). Configuration *commands* such as `color`, `bind`, and `highlight`
are not variables and cannot be modified with `:set`; set them in the
configuration file instead. To reload configuration from disk at runtime, use
`:source` (similar to the `include` configuration command).
