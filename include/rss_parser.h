...
private:
	...
	std::string
	render_xhtml_title(const std::string& title, const std::string& link);
	...
	void add_item_to_feed(
		std::shared_ptr<rss_feed> feed,
		std::shared_ptr<rss_item> item);

	void
	handle_content_encoded(std::shared_ptr<rss_item> x, rsspp::item& item);
	void
	handle_itunes_summary(std::shared_ptr<rss_item> x, rsspp::item& item);
