...
class inoreader_api : public remote_api {
	public:
		...
		virtual bool update_article_flags(const std::string& inoflags, const std::string& newflags, const std::string& guid);
	private:
...
class inoreader_urlreader : public urlreader {
	public:
		inoreader_urlreader(configcontainer * c, const std::string& url_file, remote_api * a);
